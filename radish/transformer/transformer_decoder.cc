...
#include "radish/transformer/transformer_decoder.h"
...
void TransformerDecoderImpl::reset() {
  ...
  tgt_word_emb = torch::nn::Embedding(
      torch::nn::EmbeddingOptions(options.n_tgt_vocab(), options.d_word_vec()));
  ...
  for (auto i = 0; i < options.n_layers(); i++) {
    auto layer = radish::DecoderLayer(options.d_model(), options.d_inner(),
...
void TransformerDecoderImpl::pretty_print(std::ostream& stream) const {
  stream << "transformer::TransformerDecoder(dropout=" << options.dropout()
